...
{
    internal class ScoreDBRepo: IScoreRepo
    {
        ...
        public List<Score> FindAllWithPointsInRound(long roundID)
        {
            ...
            using (var comm = dbConnection.CreateCommand())
            {
                comm.CommandText = "select r.name as round_name,p.name as name,p.id as participant_id,p.full_points as participant_points,s.points as points,s.id as score_id from Scores s inner join Rounds r on r.id=s.round_id inner join Participants p on p.id=s.participant_id where r.id=@id order by s.points DESC";
                var idParameter = comm.CreateParameter();
                idParameter.ParameterName = "@id";
                idParameter.Value = roundID;
                comm.Parameters.Add(idParameter);
                using (var result = comm.ExecuteReader())
                {
                    while (result.Read())
                    {
                        string roundName = result.GetString(0);
                        long participantID = result.GetInt64(2);
